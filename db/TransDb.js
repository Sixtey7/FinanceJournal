...
class TransDb {
    ...
    async getTransactionsForAccount(accountId) {
        ...
        try {
            ...
            let result = await client.query('SELECT * FROM ' + _pgHelper.getTransactionsTableName() + ' where data->>\'accountId\'= $1 and data->>\'type\' = \'CONFIRMED\' order by data->>\'date\'', [accountId]);
            if (result.rows) {
                return result.rows;
            }
            else {
